...
# Stage 2: Runtime (multi-process: nginx + gunicorn + node, managed by supervisord)
...
RUN apt-get update && apt-get install -y --no-install-recommends \
        nginx supervisor curl ca-certificates gosu \
    ...
    # nginx writable paths (we don't run as root)
